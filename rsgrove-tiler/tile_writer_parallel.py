...
def geojson_batch_iterator(path: str, batch_size: int, properties_schema: List[str] | None = None):
    ...
    with fiona.open(path, "r") as src:
        ...
        for i, feat in enumerate(src):
            ...
            # Expand new property keys on the fly
            for k in props.keys():
                if k not in data:
                    cols.append(k)
...
# ---------------- Parquet writer factory (compat across PyArrow builds) ----------------
def make_parquet_writer_factory(args, out_path_for):
    """
    Returns a function (path, schema) -> ParquetWriter that works across pyarrow versions.
    Only used when --format=parquet.
    """
    def make_pq_writer(path: str, schema: pa.Schema):
        # Prefer new-style WriterProperties if available
...
# ---------------- Parallel writer worker (Parquet + GeoJSON) ----------------
class IsWriterWorker(Thread):
    def __init__(self, wid: int, tasks: Queue, schema: pa.Schema,
        ...
        self.out_format = out_format  # "parquet" | "geojson"

        self._pq_writers: Dict[int, pq.ParquetWriter] = {}
        self._gj_handles: Dict[int, Tuple[object, bool]] = {}  # tile_id -> (file handle, wrote_first_feature)
    ...
    # --- GeoJSON ---
    def ensure_gj_handle(self, tile_id: int):
        if self.tile_owner(tile_id) != self.wid:
            raise RuntimeError(f"Tile {tile_id} not owned by worker {self.wid}")
        h = self._gj_handles.get(tile_id)
        if h is None:
            path = self.out_path_for(tile_id)
            os.makedirs(os.path.dirname(path), exist_ok=True)
            ...
            self._gj_handles[tile_id] = (f, False)  # wrote_first_feature=False
        return self._gj_handles[tile_id]

    def _append_geojson_features(self, tile_id: int, table: pa.Table):
        # Convert Arrow Table rows to GeoJSON Feature strings and append
        f, wrote_first = self.ensure_gj_handle(tile_id)
        cols = [n for n in table.schema.names if n != "geometry"]

        # Convert columns (except geometry) to Python lists
        props_data = {c: table[c].to_pylist() for c in cols}
        geom_bytes = table["geometry"].to_pylist()

        features_strs = []
        for i in range(len(geom_bytes)):
            g = from_wkb(geom_bytes[i])
            if g is None or g.is_empty:
                continue
            geom = shp_mapping(g)
            props = {c: props_data[c][i] for c in cols}
            feat = {"type": "Feature", "properties": props, "geometry": geom}
            features_strs.append(json.dumps(feat))

        if not features_strs:
            return

        # Write with comma handling
        if not wrote_first:
            f.write(features_strs[0])
            for s in features_strs[1:]:
                f.write(",\n")
                f.write(s)
            self._gj_handles[tile_id] = (f, True)
        else:
            for s in features_strs:
                f.write(",\n")
                f.write(s)
    ...
    def run(self):
        while True:
            ...
            if self.out_format == "parquet":
                ...
                with mem_scope("write", {"worker": self.wid, "tile": tile_id, "rows": rows, "fmt": "parquet"}):
                    w.write_table(table, row_group_size=self.row_group_size)
            else:
                with mem_scope("write", {"worker": self.wid, "tile": tile_id, "rows": rows, "fmt": "geojson"}):
                    self._append_geojson_features(tile_id, table)
    ...
    def close_all(self):
        ...
        # Close GeoJSON handles (finish the FeatureCollection)
        for tile_id, (f, wrote_first) in self._gj_handles.items():
            try:
                f.write("\n]}")
                f.close()
            except Exception as e:
...
def main():
    ...
    ap.add_argument("--suffix", default=None,
                    help="Output file suffix; default depends on --format (.parquet or .geojson)")
    ...
    # Writing / compression tuning (Parquet only)
    ...
    ap.add_argument("--compression", default="zstd",
                    ...
                    help="Parquet compression codec (parquet output only)")
    ...
    # Parquet writer factory (only used when args.format == "parquet")
    make_parquet_writer = make_parquet_writer_factory(args, out_path_for)
    ...
    if input_type == "parquet":
        ...
        scanner = dataset.scanIner(batch_size=args.batch_size)  # all columns
        ...
        for (table, xs_arr, ys_arr, bxs, bys, bxe, bye) in geojson_batch_iterator(args.input, args.batch_size):
            ...
            with mem_scope("batch", {"batch": b_idx, "rows": n}):
                ...
                if first:
                    schema = table.schema
                    queues = [Queue(maxsize=args.queue_size) for _ in range(W)]
                    workers = [IsWriterWorker(k, queues[k], schema, out_path_for, make_parquet_writer,
                                            args.row_group_size, owner_of_tile, args.format)
                               for k in range(W)]
    ...
    logger.info(f"Done. Total rows seen: {total:,}, rows written: {assigned:,} "
                f"(mode={args.mode}, tiles={P}, workers={W}, format={args.format})")
