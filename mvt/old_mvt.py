"""Legacy standalone MVT generator (kept for reference / direct invocation).

This module provides an older, single-pass approach to bucket-based MVT
generation that loads entire GeoDataFrames per parquet file.  The primary
pipeline now uses :mod:`mvt.generator` with streaming via
:class:`~mvt.streamer.GeometryStreamer`, but this module is still
functional and can be run directly as a script.
"""
